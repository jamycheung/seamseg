...
def main(args):
    ...
    val_dataloader = make_dataloader(args, config, rank, world_size)

    # Create model
    model = make_model(config, val_dataloader.dataset.num_thing, val_dataloader.dataset.num_stuff)
    if args.resume:
    ...
    # Create optimizer
    optimizer, scheduler, batch_update, total_epochs = make_optimizer(config, model, len(val_dataloader))
    if args.resume:
        optimizer.load_state_dict(snapshot["state_dict"]["optimizer"])

    # Training loop
    momentum = 1. - 1. / len(val_dataloader)
    meters = {
        ...
        "sem_conf": ConfusionMatrixMeter(val_dataloader.dataset.num_categories, momentum)
    }
